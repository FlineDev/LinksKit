...
public struct LinksView: View {
   ...
   public init(style: LinksStyle = .sections) {
      self.style = style
   }
   ...
   struct Preview: View {
      init() {
         ...
         let helpSection = LinkSection(title: "Help", entries: [
            ...
            .link(.contactSupport(email: "freemiumkit@fline.dev")),
         ])

         // Social Links
         let appSocialLinks = LinkSection(entries: [
            .link(.appOn(socialPlatform: .twitter, handle: "FreemiumKit")),
            .link(.appOn(socialPlatform: .mastodonDotSocial, handle: "FreemiumKit")),
            .link(.appOn(socialPlatform: .threads, handle: "FreemiumKit")),
         ])

         let devSocialLinks = LinkSection(entries: [
            .link(.developerOn(socialPlatform: .twitter, handle: "Jeehut")),
            .link(.developerOnMastodon(instance: "iosdev.space", handle: "Jeehut")),
            .link(.developerOn(socialPlatform: .threads, handle: "Jeehut")),
         ])
